...
export class EmployeeComponent implements OnInit, OnDestroy {
  ...
  private loadEmployees(): void {
    if (this._departmentId === undefined) {
      ...
      this.employees$ = this.employeeService.getEmployeesPaginated()
        .pipe(
          tap(employees => console.debug('Paginated employees:', employees)),
      ...
      this.employees$ = this.employeeService.getUnassignedEmployees()
        .pipe(
          tap(employees => console.debug('Unassigned employees:', employees)),
          takeUntil(this.destroy$)
        )
    } else {
      ...
      this.employees$ = this.employeeService.getEmployeesByDepartment(this._departmentId)
        .pipe(
          tap(employees => console.debug(`Employees for department ${this._departmentId}:`, employees)),
          takeUntil(this.destroy$)
        )
    }
  }
  ...
  ngOnInit(): void {
    if (this._departmentId === undefined) {
      console.debug('EmployeeComponent, loading paginated employees')
      this.loadEmployees()
